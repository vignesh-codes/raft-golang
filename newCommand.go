...
type NewEntriesState struct {
	Count int
	Mutex sync.Mutex
}

var newEntriesState = NewEntriesState{Count: 0}

func (nes *NewEntriesState) Increment() {
	nes.Mutex.Lock()
	defer nes.Mutex.Unlock()
	nes.Count++
}

func (nes *NewEntriesState) Decrement(c int) {
	nes.Mutex.Lock()
	defer nes.Mutex.Unlock()
	nes.Count -= c
}

func (nes *NewEntriesState) GetCount() int {
	nes.Mutex.Lock()
	defer nes.Mutex.Unlock()
	return nes.Count
}
